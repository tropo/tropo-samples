...
def listOptions( theContacts )
{
  ...
  theContacts.each {contact->
    if (s) s += ", "
    s += "$contact (${theContacts[ contact ].nameChoices})"
  }
  return s
}
...
def contacts = [ 	"jonathan": [ nameChoices: "Jonathan, Jonathan Taylor", number: "14074434233" ],
...
if (event.name=='choice')
{
  ...
  def ne = transfer( "sip:9${contacts[ event.value ].number}@10.6.63.201",
     ...
     onSuccess:     {event-> log("*********transfered to $event.value.calleeId")},
     onError:       {event-> log("*********transfer error")},
     onTimeout:     {event-> log("*********transfer timeout")},
     onCallFailure: {event-> log("*********transfer failed")},
     onChoice:      {event-> log("*********transfer canceled")}
     ] )
